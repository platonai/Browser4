...
const BROWSER4_GROUP_TITLE = 'Browser4';
const BROWSER4_GROUP_COLOR = 'green';
const NON_DEBUGGABLE_SCHEMES = ['chrome:', 'edge:', 'devtools:'];
const CONNECTED_BADGE = { text: '✓', color: '#4CAF50', title: 'Connected to Browser4 client' };

export function isNonDebuggableUrl(url: string | undefined): boolean {
  return !!url && NON_DEBUGGABLE_SCHEMES.some(s => url.startsWith(s));
}

// Ungroups any Browser4-titled groups left behind by a prior service worker.
export async function cleanupStaleBrowser4Groups(): Promise<void> {
  try {
    const groups = await chrome.tabGroups.query({ title: BROWSER4_GROUP_TITLE });
...
// The Browser4 tab group for an active RelayConnection. The Chrome tab group
...
export class ConnectedTabGroup {
  ...
  constructor(connection: RelayConnection, selectedTab: chrome.tabs.Tab) {
    ...
    // handshake. The relay holds Browser4-side CDP traffic until
    ...
    try {
      await this._retryOnDrag(async () => {
        if (this._groupId === null) {
          ...
          await chrome.tabGroups.update(this._groupId, { color: BROWSER4_GROUP_COLOR, title: BROWSER4_GROUP_TITLE });
        } else {
