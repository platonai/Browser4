...
```bash
# Step 1: Capture a DOM snapshot and discover the form structure
browser4-cli domsnapshot
...
> **Important:** `fill` and `select` require snapshot element refs (e.g., `e12`), not CSS selectors. Use the interactive `snapshot` command to get refs, then bridge from DOM selectors using `get attr <selector> id` or `generate-locator <ref>`. The `domsnapshot get` commands in steps 10a–10b are for **understanding** the form structure; **interacting** with it requires the accessibility-tree snapshot and its ref-based commands.
...
- [domsnapshot-scenarios-amazon.md](domsnapshot-scenarios-amazon.md) — Amazon discovery-to-extraction workflows
...
- [css-selector-bridge.md](css-selector-bridge.md) — bridging snapshot refs to DOM snapshot CSS selectors
