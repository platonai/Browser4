...
| `cli/*`                                | Browser4 CLI (`cli/browser4-cli`) |
| `skills/*`                             | AI agent skill definitions (`skills/browser4-cli`) |
...
### Test Script Portability (browser4-tests/tests-production/ and bin/test-production.ps1)

All `.ps1` and `.sh` scripts under `browser4-tests/tests-production/` and `bin/test-production.ps1` are **production test scripts** — they test a globally-installed `browser4-cli` binary, not the source tree.  They are designed to be run from **any location** (CI, a user's machine, a downloaded bundle) and must **never** depend on:
