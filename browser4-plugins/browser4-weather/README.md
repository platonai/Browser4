...
Fetch the current weather for the active page
...
| Method | Description |
|--------|-------------|
| `weather.fetchWeather` | Fetch the current weather for the active page (runs browser-side JS via WebDriver.evaluateValue) |
...
```bash
mvn -pl browser4-plugins/browser4-weather -am compile -DskipTests
mvn -pl browser4-plugins/browser4-weather package -DskipTests
```
