...
/**
 * Injects a solved Cloudflare Turnstile token into the page.
 */
open class TurnstileTokenInjector : CaptchaTokenInjector {
    private val logger = getLogger(TurnstileTokenInjector::class)
    ...
    override suspend fun inject(
        ...
        return try {
            val js = CaptchaSolveScripts.INJECT_TURNSTILE_TOKEN(token)
            ...
            logger.warn("Failed to inject Turnstile token: {}", e.message)
            false
        }
    }
}
