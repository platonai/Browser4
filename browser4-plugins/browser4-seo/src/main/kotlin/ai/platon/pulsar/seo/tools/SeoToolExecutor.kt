...
/**
 * LLM agent tool executor for the `seo` domain.
 *
 * Provides AI agents with the ability to:
 * - `seo.extractMeta()` — extract all SEO metadata from the current page
 * - `seo.checkIssues()` — audit the current page for common SEO problems
 *
    ...
    init {
        ...
        toolSpec["checkIssues"] = ToolSpec(
            ...
            help = """
                ...
                Use this after seo.extractMeta() to get actionable recommendations.
            """.trimIndent()
        )
    }
    ...
    override suspend fun callFunctionOn(
        ...
        val driver = receiver as? WebDriver
            ?: throw IllegalArgumentException(
                "seo.$functionName requires a WebDriver receiver (current page context)"
            )
        ...
        return when (functionName) {
            ...
            "checkIssues" -> seoService.checkIssues(driver)
                ...
                "Unsupported seo method: $functionName. Supported: extractMeta, checkIssues."
            )
        }
    }
}
