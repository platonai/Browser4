...
```json
{"type":"object","required":["steps","failureCount","stoppedOnError","cached","cachedSteps","durationMs"],"properties":{"steps":{"type":"array","items":{"type":"object","required":["index","id","tool","ok","durationMs"],"properties":{"index":{"type":"integer"},"id":{"type":"string"},"tool":{"type":"string"},"ok":{"type":"boolean"},"durationMs":{"type":"integer"},"text":{"type":"string"},"errorCode":{"type":"string"},"error":{"type":"string"},"cached":{"type":"boolean"}}}},"failureCount":{"type":"integer"},"stoppedOnError":{"type":"boolean"},"cached":{"type":"boolean"},"cachedSteps":{"type":"integer"},"durationMs":{"type":"integer"}}}
```
...
- Analyse a page and promote what it teaches: `{"url": "https://example.com/product/1", "intent": "buy the product"}`
  - Runs analysis tools against the current page and takes seconds, not milliseconds.
...
- List what is known about one domain: `{"filter": "example.com", "page_size": "50"}`
  - Every argument is optional — `experience_list` with no arguments lists everything.

### `experience_query`

```
experience.query(url: String, intent: String? = null)
```
...
- Ask what is already known about a page: `{"url": "https://example.com/product/1", "intent": "buy the product"}`
  - Omit `intent` to let the stored (domain, url) facts answer.

### `experience_save`

```
experience.save(url: String, trace: String, outcome: String = success, intent: String? = null, task_type: String? = null, facts: String? = null)
```
...
| `intent` | String? | no | null | Free-text intent of the task. |
| `task_type` | String? | no | null | Task classification, when the caller knows it. |
...
- Record what a finished task learned: `{"url": "https://example.com/product/1", "trace": "{"steps":[{"tool":"click","selector":"#buy"}],"outcome":"success"}", "intent": "buy the product"}`
  - `facts` is optional; pass it to merge selectors, blockers or anti-patterns.
...
- List the installed skills: no arguments
...
- Compact accessibility snapshot: `{"compact": "true"}`
...
- Click a button: `{"selector": "#submit"}`
- Click a row twice: `{"selector": "#row-1", "count": "2"}`
...
- Clear the console buffer: no arguments
...
- Read console output: no arguments
...
- Double click a cell: `{"selector": "#cell-1"}`
...
- Accept a pending dialog: no arguments
...
- Dismiss a pending dialog: no arguments

### `dialog_status`, `browser_dialog_status`

```
tab.dialogStatus()
```

Report the pending JavaScript dialog, if any: `{pending, type, message}`. Read-only — it never dismisses the dialog.
...
```json
{"type":"object","required":["pending"],"properties":{"pending":{"type":"boolean"},"type":{"type":"string"},"message":{"type":"string"}}}
```
...
- Check for a pending dialog: no arguments
...
- Read the document URI: no arguments
...
- Evaluate an expression: `{"expression": "document.title"}`
...
- Read the page title in JS: `{"expression": "document.title"}`
...
- Return to the main frame: no arguments
...
- Enter an iframe: `{"frame": "#pay-frame"}`
...
- Start recording network traffic: `{"contentMode": "embed"}`
...
- Stop HAR recording: no arguments
...
```
tab.isEnabled(selector: String)
```

Whether the element matched by `selector` is enabled — not disabled and not read-only.

| Argument | Type | Required | Default | Meaning |
|---|---|---|---|---|
| `selector` | String | yes |  | Element to inspect. |

Returns: `Boolean`

Examples:

- Is the submit button enabled?: `{"selector": "#submit"}`
...
- Restore a saved session: `{"state": "{"cookies":[],"origins":[]}"}`
...
- Dump the DOM as YAML: no arguments
...
- List network requests: no arguments
...
- Stub an API response: `{"urlPattern": "**/api/ads", "body": "{}", "contentType": "application/json"}`
...
- Remove an API stub: `{"urlPattern": "**/api/ads"}`
...
- Read the page HTML: no arguments
...
- Print the page to PDF: no arguments
...
- Submit with Enter: `{"key": "Enter", "selector": "#q"}`
...
- Read the referrer: no arguments
...
- Save cookies and localStorage: no arguments
...
- Capture one element: `{"selector": "#chart"}`
...
- Scroll to the bottom: no arguments
...
- Scroll to the top: no arguments
...
- Type into a field: `{"text": "hello", "selector": "#comment"}`
...
- Wait for a client-side condition: `{"pageFunction": "document.querySelectorAll('.row').length > 10"}`
...
- Wait for the page after leaving a URL: `{"oldUrl": "https://example.com/login", "timeoutMillis": "10000"}`
...
- Wait for a URL to load: `{"url": "https://example.com/next", "timeoutMillis": "15000"}`
...
- Wait for results to render: `{"selector": "#results", "timeoutMillis": "5000"}`
