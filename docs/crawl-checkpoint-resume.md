...
One file per task, outside the task store and outside its lifecycle:

```
${browser4.data.dir:-$HOME}/.browser4/data/crawl/checkpoints/<taskId>.json
${browser4.data.dir:-$HOME}/.browser4/data/crawl/checkpoints/<taskId>.json.bak
```
...
2. **The work state** — per seed: the rows already fetched, the URLs that failed
   terminally (with their reasons), the URLs that were submitted and never settled,
   and the counts (`pagesExpected`) the merged report is built from.
3. **The frontier** — links discovered at `depth < maxDepth` that the round could not
   hand to the session because it had already ended. This is what lets a `--depth >= 1`
   crawl continue from where it stopped instead of walking the site again.

### Crash safety and the write policy

The file is written by **atomic replace**: the new state goes to a `.tmp` file, is
flushed, and is then moved over the live file, with the previous good copy kept as
...
Writes are periodic, not only on status changes:

| Trigger | When |
|---|---|
| Change | every 5 settled URLs |
| Staleness | every 2 s, even if nothing settled |
...
task), because the file is a whole-state rewrite: a small crawl writes as soon as
enough changed, a very large one writes proportionally less often. The consequence is
stated plainly: a `SIGKILL` can cost the URLs settled since the last write. For a
normal crawl that window is a handful of URLs or a couple of seconds — a resumed
crawl does not re-request work it had already finished; for a very large crawl the
window is wider, and the alternative would be re-writing megabytes several times a
second.
