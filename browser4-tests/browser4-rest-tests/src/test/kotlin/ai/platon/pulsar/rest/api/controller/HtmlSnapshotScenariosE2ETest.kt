...
private val createdSessions = mutableListOf<String>()
    ...
    /** Wait for the page to have any non-empty title (mock content varies). */
    private fun awaitAnyTitle(sessionId: String, timeoutMs: Long = 15000) {
        val deadline = System.currentTimeMillis() + timeoutMs
        var lastTitle = ""
        while (System.currentTimeMillis() < deadline) {
            val result = callTool(
            ...
            if (!result.isError) {
                lastTitle = textContent(result)
                if (lastTitle.isNotBlank() && lastTitle != "localhost") return
            }
            Thread.sleep(500)
        }
        throw AssertionError(
            "Timed out waiting for non-blank page title, last: '$lastTitle'"
        )
    }
    ...
    fun test4b_queryMultipleUrls() {
        ...
        val title2 = r2[0]["title"]?.asText().orEmpty()
        assertTrue(title2.isNotBlank(), "Expected second product title to be non-blank")
        assertTrue(title1 != title2, "Expected different product titles, got same: '$title1'")
    }
