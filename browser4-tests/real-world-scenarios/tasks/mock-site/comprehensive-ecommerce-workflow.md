...
6. Use htmlsnapshot inspect with `--max 3 --depth 2` to discover the CSS selectors for the product title, price, description, and image.
7. Use htmlsnapshot get all to extract all text content from the product description area.
8. Write an X-SQL query to a file that extracts the product title, price, and image URL. Run it with `--sql @file`.
9. Use htmlsnapshot grep to search the page HTML for price-related patterns.
...
12. On the listing page, capture an HTML snapshot and run htmlsnapshot get all to extract all product titles and prices.
