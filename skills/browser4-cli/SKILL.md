...
| `network requests`, `network request <id>`, `network har start`, `network har stop` | Network request inspection & HAR recording | Inspect what the page loaded (XHR/fetch/status/headers), debug API calls, record a `.har` file (Chrome DevTools importable). `network requests --filter api --status 2xx`; `network har start --content text` then `network har stop ./capture.har` | [network.md](references/network.md) |
