...
* 👽 **浏览器智能体** — 全自主浏览器智能体，能够推理、规划并执行端到端任务。
* 🤖 **浏览器自动化** — 高性能自动化，适用于工作流、导航和数据提取。
* ⚙️ **机器学习智能体** — 在不消耗 token 的情况下学习复杂页面的字段结构。
* ⚡ **极致性能** — 完全协程安全；单机每日支持 10 万 ~ 20 万次复杂页面访问。
* 🧬 **数据提取** — 混合 LLM、ML、X-SQL 和选择器，在混乱的页面中提取干净数据。
...
手动安装是可选的，因为你的 AI 智能体在阅读 SKILL 后已经足够聪明，可以自行安装。
...
Browser4 CLI 是一个功能强大的命令行界面，用于直接浏览器控制和自动化，专为人类用户和 AI 智能体设计。它提供简洁的语法来执行复杂的浏览器交互，无需编写代码。

Browser4 CLI 兼容 Playwright，支持导航、交互和数据提取等各种命令。它可以在脚本、终端会话中使用，或通过 SKILLS 集成到 AI 智能体中。

命令设计直观且可组合，允许你将多个操作串联起来完成复杂的工作流。

Browser4 CLI 专为 AI 智能体通过 SKILLS + CLI 使用而设计 — 参见 [SKILL.md](skills/browser4-cli/SKILL.md)。
...
```
-s <name>, --session <name>    命名的会话标签
--server <url>                 覆盖 Browser4 服务器 URL
--json                         输出机器可解析的 JSON 到 stdout
-q, --quiet                    抑制正常输出，仅显示错误
...
```
open [url]        打开浏览器会话，可选地导航到 URL
                   --headed, --headless, --profile <path>, --profile-mode <mode>
attach            通过 CDP 连接到现有浏览器（--cdp <channel|url|port>）
close             关闭当前浏览器会话
close-all         关闭所有浏览器会话而不停止后端
...
press <key> [ref]          在焦点元素或目标 ref 上按键。--verify, --follow（检测新标签页）
select <ref> <value>       在下拉菜单中选择选项。--verify
check <ref>                选中复选框或单选按钮
uncheck <ref>              取消选中复选框或单选按钮
drag <startRef> <endRef>   在两个元素之间拖放
upload <ref> <file>        上传文件到文件输入框
wait [target]              等待条件：元素、时间（--text）、URL（--url）、页面加载（--load）或 JS（--fn）
```
...
`htmlsnapshot` 系列命令捕获原始 HTML DOM，用于 CSS 选择器和 X-SQL 查询 — 无需交互式浏览器会话。
...
交互操作           支持 click, type, fill 不支持
X-SQL 支持         不支持                 支持 (query)
```

```
htmlsnapshot capture                        捕获静态 HTML 快照并存储到页面存储中
htmlsnapshot                                上述命令的简写形式
htmlsnapshot get <field> [selector] [name]  从存储的 HTML 快照中提取 text、html 或 attr
htmlsnapshot query [url]                    对存储的 HTML 快照运行 X-SQL（--sql <query|@file>）
...
完整参考（包括同样需要 LLM 密钥的 X-SQL `llm_*` 函数），请参见 [HTML 快照参考](skills/browser4-cli/references/htmlsnapshot.md)。
...
火山引擎 (ByteDance)        VOLCENGINE_API_KEY, VOLCENGINE_MODEL_NAME, VOLCENGINE_BASE_URL
OpenAI 兼容                 OPENAI_API_KEY, OPENAI_MODEL_NAME, OPENAI_BASE_URL
阿里云通义千问 (DashScope)    OPENAI_API_KEY, OPENAI_MODEL_NAME, OPENAI_BASE_URL
```

这些环境变量映射到 [application.properties](application.properties) 中的相应属性。示例：

```bash
export DEEPSEEK_API_KEY=sk-xxxxxxxxxxxxxxxxxxxxxxxx
```

如果没有配置有效的 LLM 密钥，AI 命令将在启动时快速失败并显示清晰的错误信息。

#### AI / 智能体

> **需要 LLM API 密钥** — 参见上方的 [LLM 配置](#llm-配置)。

提交自然语言任务，让 Browser4 的后端 AI 智能体自主规划和执行：

```
agent run <task>          提交自主任务（异步，立即返回任务 ID）
agent status <id>         检查运行中的智能体任务状态
agent result <id>         获取已完成智能体任务的最终结果
extract <instruction>     使用 AI 从页面提取结构化数据。--schema <json>, --filename, --raw
summarize [instruction]   使用 AI 总结页面内容。--selector, --filename, --raw
```

- `agent run` 是异步的 — 后端智能体推理、探索并执行，直到任务完成。
- 智能体命令基于任务 ID，不需要活跃的 CLI 浏览器会话槽。
...
swarm submit [url]    提交 URL 或 X-SQL 负载作为抓取任务
                      --seed-file, --sql, --deadline, --expires, --refresh, --parse, --store-content
swarm query <url>     提交 X-SQL 查询以从已加载的网页中提取数据
                      --sql, --seed-file, --deadline, --expires, --refresh
swarm status <id>     检查抓取任务的状态
swarm result <id>     获取已完成抓取任务的结果
```

种子文件为纯文本格式，每行一个 URL；`#` 注释和空行会被忽略。在 X-SQL 模板中使用 `@url` — 它会在服务端被替换为目标 URL。

#### Crawl（爬取）

```
crawl [url]     从 URL 或种子文件爬取网站，可选进行 X-SQL 提取
                --seed-file（批量 URL 列表，每行一个，# 注释会被忽略）
...
batch <command...>  在单个进程中执行多个命令
                    --bail（首次失败即停止），--json（从 stdin 读取 JSON 命令）
loop [task]         按间隔重复执行任务
...
某些命令可能需要比默认 HTTP 超时更长的时间。使用以下环境变量调整超时时间：
...
文本输入命令使用较长的默认超时时间，因为在表单字段中输入文本 — 尤其是在复杂页面上 — 可能比简单交互更慢。如果文本输入命令超时，操作**可能已部分执行**。超时后，在重试之前使用 `snapshot` 或 `get` 验证字段内容。
...
# AI 驱动的提取和总结（需要 LLM 密钥 — 参见上方的 LLM 配置）
...
**前置条件：** Git、JDK 17+（推荐 21+）、Chrome/Chromium，以及 PowerShell 7（仅 Linux/macOS）。完整的前置条件表格、平台特定工具和 Chrome 自动检测路径，请参见 [从源码构建](docs/build-from-source.md)。
...
2. **配置 LLM API 密钥**
   ...
   > 编辑 [application.properties](application.properties) 并添加你的 API 密钥，或设置环境变量。参见 [LLM 配置](#llm-配置) 了解支持的提供商和变量名称。
...
由自监督/无监督机器学习驱动的大规模、高精度、自动化字段发现与提取 — 无需 LLM API 调用、不消耗 token、确定性且快速。

**它能做什么：**
- 学习商品/详情页面上每个可提取的字段（通常数十到数百个），精度极高。
- 当 Browser4 在 GitHub 上达到 10K 星标时开源。

**为什么不只用 LLM？**
- LLM 提取会增加延迟、成本和 token 限制。
- 基于 ML 的自动提取是本地化的、可复现的，并且可扩展到每天 10 万 ~ 20 万页面。
- 你仍然可以将两者结合：使用自动提取作为结构化基线 + LLM 进行语义丰富。
...
- 原生 Browser4 API 暴露正在计划中；请关注发布更新。

**核心优势：**
- 高精度：>95% 字段被发现；大多数准确率 >99%（在测试域名上的指示性数据）。
- 对选择器变动和 HTML 噪声具有弹性。
- 零外部依赖（无需 API 密钥）→ 大规模使用成本效率高。
...
Browser4 包含一个轻量级 **MockSite** 服务器，用于提供静态 HTML 页面以进行测试和演示。从仓库根目录启动：
...
完整的文档可在 `docs/` 目录和我们的 [GitHub Pages 站点](https://platonai.github.io/browser4/) 上获取。
...
设置环境变量 `PROXY_ROTATION_URL` 为你的代理服务提供商提供的轮换 URL：
