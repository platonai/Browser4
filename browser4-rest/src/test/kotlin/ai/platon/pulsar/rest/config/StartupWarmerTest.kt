 ...
 * from HTTP request → MCP dispatch → session → browser.
 *
 ...
 * updating the warmer), which would reintroduce cold-start latency with
 * [spring.main.lazy-initialization=true].
 *
 * `protocolFactory` / `fetchComponent` are intentionally absent — the
 ...
 * absent too: they are non-primary scenarios with heavy initialization and
...
class StartupWarmerTest {
    ...
    fun `onReady continues warming after a bean fails`() {
        val context = Mockito.mock(ApplicationContext::class.java)
        Mockito.`when`(context.getBean("agenticContext"))
            .thenThrow(RuntimeException("simulated lazy-init failure"))

        val warmer = StartupWarmer(context)
        warmer.onReady() // must not throw synchronously

        // All 3 beans must still be attempted — the warm-up must not
