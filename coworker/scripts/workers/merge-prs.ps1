...
try {
    ...
    $conflictResolved = @()
    $skipped = @()
    $ktJavaMerged = @()   # subset of merged PRs that touched .kt/.java files
    $ktJavaChanges = @{}

    # Pre-check: which PRs touch .kt/.java files (relevant for CI threshold)?
    Write-Host "`n── Checking for Kotlin/Java file changes... ──" -ForegroundColor DarkGray
    foreach ($pr in $prs) {
        $prNum = $pr.number
        $prFiles = & gh pr view $prNum --json files --jq '.files[].path' 2>&1
        if ($LASTEXITCODE -eq 0) {
            $ktJavaFiles = @($prFiles | Where-Object { $_ -match '\.(kt|java)$' })
            if ($ktJavaFiles.Count -gt 0) {
                $ktJavaChanges[$prNum] = $true
                Write-Host "  PR #${prNum}: $($ktJavaFiles.Count) .kt/.java file(s) — counts toward CI threshold" -ForegroundColor DarkGray
            } else {
                $ktJavaChanges[$prNum] = $false
                Write-Host "  PR #${prNum}: no .kt/.java changes — excluded from CI threshold" -ForegroundColor DarkGray
            }
        } else {
            # Can't check — assume relevant to be safe
            Write-Host "  PR #${prNum}: could not list files, assuming .kt/.java changes" -ForegroundColor Yellow
            $ktJavaChanges[$prNum] = $true
        }
    }
    ...
    foreach ($pr in $prs) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $merged += $prNum
            if ($ktJavaChanges[$prNum]) { $ktJavaMerged += $prNum }
            & git pull origin $BaseBranch 2>&1 | Out-Null
            continue
        }
...
"@
    ...
    Write-Host "  .kt/.java changes:    $($ktJavaMerged.Count) $($ktJavaMerged -join ', ')" -ForegroundColor Magenta

    # ── Update CI merge counter (only .kt/.java PRs count toward threshold) ──
    $totalMergedThisRun = $merged.Count + $conflictResolved.Count
    $ktJavaThisRun = $ktJavaMerged.Count
    $ciState = Get-CiMergeState
    $mergedSinceLastCi = $ciState.mergedSinceLastCi + $ktJavaThisRun
    Set-CiMergeState -Count $mergedSinceLastCi -LastTriggered $ciState.lastCiTriggered

    Write-Host "PRs merged since last CI: $mergedSinceLastCi ($ktJavaThisRun this run, .kt/.java only, threshold: $CiThreshold)" -ForegroundColor DarkGray
