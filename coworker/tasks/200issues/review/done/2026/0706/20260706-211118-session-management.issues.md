...
A command like `cargo run -- -s research info` or `cargo run -- sessions detail research` that shows a session's current URL, title, number of tabs, uptime, and snapshot history.

#### Actual Behavior

The only session-inspection tool is `list`, which shows a summary table. To check what page a session is on, you must run `snapshot` (which creates a new snapshot file). There is no read-only "what's the current page?" command that doesn't change state.

#### Root Cause Analysis

The session management command surface is minimal — no dedicated introspection command exists.

#### Code Pointer

``cli/browser4-cli/src/` — session management commands module.`
...
Either inline content preview (first N nodes) on stdout, or a clear note that content was written to a file with instructions for viewing it.

#### Actual Behavior

The output shows `[Snapshot](D:\workspace\...\snapshot-....yml)` — a file path in a markdown link format, and a summary line like "Snapshot size: 31 KB (472 nodes/lines)". The user must manually locate and read this file to see what elements are available. The `--stdout` flag (found in the `help` output of `snapshot --help`) is not suggested.

#### Root Cause Analysis

The default `snapshot` output prioritizes file-path display over inline content, which makes sense for large pages but leaves a UX gap for quick verification.

#### Code Pointer

``cli/browser4-cli/src/` — snapshot rendering logic.`
...
"Session ID" should show a distinct identifier (UUID, numeric ID, or different internal identifier) that distinguishes it from the display name.

#### Actual Behavior

For the "research" session, both "Name" and "Session ID" show "research". For "(default)", "Session ID" shows "DEFAULT". The column conveys no additional information for named sessions.

#### Root Cause Analysis

Named sessions use the session name as the session ID, making the two columns redundant.

#### Code Pointer

`Backend session management — session naming/id strategy.`
