...
#### Reproduction
```bash
# From repo root; files at ./
# But cargo run CWD is cli/browser4-cli/
swarm query --sql @extract-query.sql --seed-file seed-urls.txt --refresh
# File not found
```

#### Expected Behavior
Clear error messages showing resolved absolute path, or documentation explaining CWD behavior.

#### Actual Behavior
Relative paths resolve against `cli/browser4-cli/`. Users must discover `../../` prefix through trial and error. Error messages may not clearly indicate resolution failure.

#### Root Cause Analysis
`cargo run` sets CWD to crate directory. `@file` syntax resolves relative to CWD without helpful error messaging.

#### AI Suggested Improvement
- Show absolute path tried when file is not found
- Document CWD behavior in swarm reference and SKILL.md
...
#### Expected Behavior
Absolute URL like `http://localhost:18080/ec/static/img/placeholder.png`.

#### Actual Behavior
Relative path `/ec/static/img/placeholder.png`. Users must manually prepend base URL.

#### Root Cause Analysis
`DOM_FIRST_ATTR` returns raw attribute value. `DOM_FIRST_IMG` (documented as returning absolute src) returns empty due to known bug. No `DOM_ABS_ATTR` function exists.

#### AI Suggested Improvement
- Fix `DOM_FIRST_IMG` to return absolute URLs as documented
- Add `DOM_FIRST_ABS_ATTR(DOM, selector, attr)` for URL resolution
