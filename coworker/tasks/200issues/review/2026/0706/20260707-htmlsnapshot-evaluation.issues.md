# browser4-cli htmlsnapshot evaluation — Issues

Evaluation of htmlsnapshot workflow on `http://books.toscrape.com/`. 10-step task completed successfully. 8 issues found.
...
Remove support for `--offset` and `--limit`, since it already support `--page` and `--page-size`.
