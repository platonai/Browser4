...
- **Fix status:** ✅ Applied 2026-07-07 — Added `eprintln!("💡 Tip: Run snapshot -v 0 to see interactive element refs")` in `post_command_snapshot()` at `main.rs:880`
...
- [ ] **REJECT** — issue invalid, not a problem, or already addressed
- **Notes:**
- **Fix status:** ✅ Applied 2026-07-07 — (1) Made `--stdout` tip always visible (removed `--interactive` guard); (2) Added brief preview of first 10 non-comment snapshot lines in default output mode at `main.rs:2738-2760`
...
- [ ] **REJECT** — issue invalid, not a problem, or already addressed
- **Notes:**
- **Fix status:** ✅ Already fixed (pre-existing) — `convert_alternation()` at `main.rs:4788-4797` already uses `AtomicBool` to log the conversion note only once per session. Verified 2026-07-07, no additional change needed.
