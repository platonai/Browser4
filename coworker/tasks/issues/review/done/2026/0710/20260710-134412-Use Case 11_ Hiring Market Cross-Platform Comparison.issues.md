...
Page should load with job listings or search interface.

#### Actual Behavior

All `/jobs` and `/role` pages return a DataDome CAPTCHA interstitial. The page title is generic "wellfound.com" and the snapshot contains only `Iframe "DataDome CAPTCHA"`. Even fresh named sessions (`-s`) are immediately blocked. Only the marketing homepage (`wellfound.com/`) loads without CAPTCHA.

#### Root Cause Analysis

Wellfound uses DataDome anti-bot protection which detects headless/automated Chrome instances via fingerprinting (navigator.webdriver, CDP runtime flags, missing browser features). The browser4-cli backend launches Chromium with flags that are detectable by enterprise-grade bot detection. No documentation addresses CAPTCHA handling or bot detection evasion.

#### Code Pointer

``browser4-browser/` — the Chromium launch configuration (likely missing stealth/anti-detection flags)`
...
A clean new browser session, or at minimum a clear indication that a previous session's state (cookies, localStorage, URL history) is being reused.
...
The default session (no `-s` flag) persists across CLI invocations and auto-reconnects. The `list` command shows session status with a "Next open" column, but this behavior is not obvious to a first-time user who may not know to check for existing sessions.

#### Code Pointer

`Session management in the CLI — default session persistence logic`
...
The snapshot content should be directly visible or easily accessible in the terminal output.

#### Actual Behavior

The default output truncates at 10 lines with an ellipsis, requiring the user to run a separate command (`--stdout`), open a file, or use `snapshot grep`. For a first-time user, this adds friction to the core workflow. The SKILL.md's templates show `snapshot -v 0` without `--stdout`, implying the output would be visible inline.

#### Root Cause Analysis

The YAML snapshot file can exceed 256KB, so the CLI truncates terminal output as a safeguard. However, the truncation threshold (10 lines) is too aggressive for typical pages where the interactive elements are in the first 20-50 lines.

#### Code Pointer

``cli/browser4-cli/src/snapshot.rs` — the preview truncation logic`
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
keep compatible with standard grep
...
Documentation should acknowledge the limitation, list known-incompatible sites, and provide workaround strategies.

#### Actual Behavior

Zero mentions of CAPTCHA, bot detection, or anti-automation systems anywhere in the documentation. The `wellfound-captcha.png` file in the repo root suggests the team encountered this issue before but didn't document it. A first-time user hitting this has no guidance on what to do.

#### Root Cause Analysis

The documentation focuses on the happy path (unprotected sites) and doesn't cover failure modes. The development.md mentions "output redirection" and other dev-mode concerns but not bot detection.

#### Code Pointer

``skills/browser4-cli/SKILL.md` — add a "Known Limitations" section`
...
The working directory should persist between commands, allowing shorthand invocation.

#### Actual Behavior

Every Bash invocation resets CWD to `C:\Users\pereg`. This forces all browser4-cli commands to be prefixed with `cd "D:/workspace/Browser4/Browser4-4.11" && cargo run --manifest-path cli/browser4-cli/Cargo.toml --` — a 100+ character prefix that adds significant friction and visual noise.

#### Root Cause Analysis

The evaluation harness (Claude Code on Windows with Git Bash) resets CWD between tool calls. This is a platform/environment issue but significantly impacts the CLI experience during evaluation.

#### Code Pointer

`N/A — environment/harness issue`
...
Each command prints `Finished 'dev' profile [unoptimized + debuginfo] target(s) in 0.XXs`. While the build is cached and fast (0.2-0.8s), the "Finished" message appears on every invocation, adding noise and the perception of slowness.

#### Root Cause Analysis

Cargo always checks timestamps and re-links even when nothing changed. The `--quiet` flag suppresses this but also suppresses all non-error output.

#### Code Pointer

`development.md mentions `--quiet` for output redirection`
