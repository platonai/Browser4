...
All 9 steps completed and verified. Here are the deliverables.
...
Workarounds needed: none blocking. `--sql @file` and `--seed-file` passed through Git Bash → pwsh without quoting problems.

Two investigation notes worth recording. First, I initially concluded that `b4w.ps1` swallows exit codes — that was **wrong**: I had piped to `tail`, so `$?` reported `tail`'s status. Re-tested with direct redirection, the wrapper propagates correctly (`config set timeout 0` → 1, unknown command → 2). The genuine finding is narrower (Issue 3). Second, my first CSV write had to be diffed against the extraction dump to rule out transcription slips; it matched.
