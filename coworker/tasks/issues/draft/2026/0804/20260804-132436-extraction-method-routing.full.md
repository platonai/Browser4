...
**Commands used:** `goto`, `snapshot -v 0 --stdout`, `snapshot -v 1 --stdout`, `fill`, `select`, `check`, `click`, `reload`, `htmlsnapshot`, `htmlsnapshot get text`, `htmlsnapshot get all text`, `htmlsnapshot inspect`, `htmlsnapshot query --sql @file`, `htmlsnapshot grep`, `eval --stdin`, `eval --stdin --await`, `eval --stdin --json`, `extract`, `crawl --seed-file --depth 0 --sql @file --format table --refresh`
