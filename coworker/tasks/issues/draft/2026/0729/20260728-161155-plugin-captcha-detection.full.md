...
**Commands used**: `help`, `plugin list`, `plugin install`, `stop`, `open`, `plugin-captcha` (×2), `goto` (×5), `snapshot -v 0 --stdout`, `eval --file` (×7), `doctor log`, direct `curl` to `/mcp/call-tool` for `captcha_getBalance` and `captcha_detect`.
