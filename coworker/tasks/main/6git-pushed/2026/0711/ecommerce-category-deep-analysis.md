...
> **Review complete:** 4 approved, 4 deferred/rejected
...
**Summary:** - Replace template variables with literal commands in the evaluation template, or add a short "Setup" block at the top that defines them explicitly
...
**Summary:** - Add a `scroll-into-view <ref|selector>` command that uses `Element.scrollIntoView()` to trigger intersection-observer-based lazy loading
...
**Summary:** - Add a `bin/dev-cli.ps1` / `bin/dev-cli.sh` wrapper script that sets up the invocation
...
**Summary:** - Add a scenario to `htmlsnapshot-scenarios-amazon.md` for "top-N product extraction workflow" showing the full pattern
