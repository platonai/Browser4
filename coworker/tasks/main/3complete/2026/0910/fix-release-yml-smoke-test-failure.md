...
**Redundant — stale-run race, no new failure.** The diagnostics above point at
run 34439006612, which was already root-caused, fixed and superseded:

- Root cause (fixed in `0f2a3a292f`): `cli/scripts/smoke-test-runtime-bundle.sh`
  started `python3 -m http.server` without `--directory "$TEMP_DIR"`, so the
  browser browsed Python's 404 page and `type hello-smoke #input1` failed with
  "no element found for selector [#input1]". The backport `8eabd1acf2` makes
  `type` fail loudly on a missing selector, which exposed that latent harness bug.
- When `monitor-release.ps1` was re-run to publish from the fixed commit, its
  "locate the triggered run" step matched run 34439006612 (already completed with
  failure) a few seconds before GitHub listed the freshly triggered run, so it
  reported failure and filed this task.
- `v4.13.17` was re-pointed at the fix commit and the release re-triggered as run
  34442693881. No further code change is required.
