...
- **Notes:**
[AI suggested: ACCEPT] The template genuinely misleads first-time users into silently receiving partial data — the suggested replacement with `get all text` plus a one-line `get` vs `get all` distinction fixes it with no semantic change. The optional CLI hint for `--all` + `get` is a nice-to-have that can be filed separately.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] This is a docs-vs-behavior contradiction in the reliability-critical direction: scripts treat exit 0 as success, so empty extraction is a silent failure in CI. Fix the code to exit non-zero on no-match (matching the already-documented contract) rather than weakening the docs; the `--json` `"matched"` field is optional and can be deferred.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] `--stdout` printing a double-encoded envelope that also falsely reports `"completed":false` directly contradicts the help text and breaks script consumption of `extract`. Unwrapping `ExtractResult.description` in raw mode is the correct fix; pair it with Issue 5's change since both touch the same `handle_extract` path.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT with improvements] The undocumented CWD-relative `../../../../../../.browser4-cli/snapshot` dir is a real source-tree-pollution and discoverability gap; documenting it and adding an env override (BROWSER4_CLI_SNAPSHOT_DIR or a config key) is low-risk. The proposed default relocation to `~/.browser4/snapshot` is a breaking change for any existing consumers of the current path — defer that part or deprecate gradually.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Writing a file unconditionally when `--stdout` was explicitly requested contradicts the help text and pollutes the working tree. Skipping `save_snapshot` under `--stdout`/`--raw` is a small, contained fix — coordinate it with Issue 3 in the same `handle_extract` change, and note the `--filename`-only-write refinement applies to the broader snapshot path too.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Quick doc fix in the §4a decision tree — replace the misleading "auto-discovers content" line with `htmlsnapshot inspect` or clarify `article` is merely an example tag selector. Related to Issue 1 (same SKILL.md file, same `get` semantics confusion) but a distinct defect; fix both in one documentation pass.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Documenting `--format json|csv|table` and `--result-only` in SKILL.md §4e and htmlsnapshot.md is a straightforward discoverability fix for a real feature that exists but is hidden. The TTY-default suggestion is a product-behavior change — evaluate separately rather than bundling it into the doc fix.
