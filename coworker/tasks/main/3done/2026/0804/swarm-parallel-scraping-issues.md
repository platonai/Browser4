...
- **Notes:**
[AI suggested: ACCEPT] Appears already fixed in current code (line ~16682 — `let bin` is on its own line). The CI suggestions (cargo check on commit, pre-commit hook) are good defensive measures to prevent recurrence.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Well-diagnosed root cause. X-SQL evaluator should validate argument types before invocation and produce clear composition errors. The suggestion to make `DOM_ABS_SRC` auto-detect string vs DOM element is a pragmatic defense-in-depth approach. Strongly related to Issue 3.
...
- **Notes:**
[AI suggested: DUPLICATE] Same root cause as Issue 2 — uncaught X-SQL evaluation exceptions. Fixing the error propagation in the X-SQL evaluator (Issue 2) will naturally populate the `message` field. Consolidate into Issue 2's fix scope.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] `isDone` should reflect terminal state, not just success. Any task with a finish time should have `isDone: true`. Simple fix in the status response builder — gate on lifecycle state (completed/failed/cancelled) rather than statusCode 200.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Genuine reliability issue. Audit the job dispatcher for dequeuing fairness. The symptom (jobs stuck minutes while newer ones complete) suggests non-FIFO behavior or per-type queue partitioning. Related to Issues 4 and 6 in that all three are swarm task lifecycle reliability bugs.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Classic TOCTOU race — result persistence and status transition aren't atomic. Fix by writing results before marking the task complete, or using a transaction. Related to Issues 4 and 5 (swarm task lifecycle reliability cluster).
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Valid documentation gap that directly contributes to users hitting Issues 2/3. The function index should distinguish DOM-element functions from scalar-returning functions, and show composability constraints. A composition matrix or visual graph would be most effective.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT with improvements] The suggested auto-clear on create is too aggressive (could lose data the user intended to keep). Instead: make the warning more prominent (color/bold), add an interactive "Clear now? [Y/n]" prompt, and include `--clear-stale` directly in the warning text as already done.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Simple fix — sort by submission time or task ID on the backend or CLI side. Grouping by status (queued→processing→done→failed) would further improve scanability.
...
- [ ] **DUPLICATE** — issue duplicates another existing issue (reference in Notes)
- **Notes:**
[AI suggested: ACCEPT] Valid pain point but lowest priority of the set. Caching a source hash to skip redundant builds is the right approach. Consider also adding a `--no-build` flag for scripts that know the binary is current.
