...
| Issue | Status | Change |
|---|---|---|
| 1 (--clear discoverability) | Partially fixed | `agent list --help` documents `--clear`; top-level agent help still omits it |
| 2 (--clear no warning) | Deferred | UX; clearing in-progress tasks silently |
| 3 (fictional task IDs) | Fixed | agent.md examples updated to UUID format |
| 4 (stale task persistence) | Deferred | UX |
| 5 (status JSON verbose) | Deferred | UX |
| 6 (status casing) | Fixed | friendly_agent_status maps to lowercase "completed" |
| 7 (agent clear command) | Deferred | UX; `agent list --clear` exists |
| 8 (help old status format) | Fixed | agent.md updated (RUNNING example remains accurate for backend JSON) |
