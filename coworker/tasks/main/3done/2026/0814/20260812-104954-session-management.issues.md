...
| Issue | Status | Change |
|---|---|---|
| 1 (first-launch latency) | Fixed | SKILL.md first-run note now includes the Maven build step |
| 2 (multi-session workflow) | Fixed | SKILL.md §6 added a Multi-Session Workflow quick pattern |
| 3 (snapshot -i output size) | DEFER | pre-existing human decision |
| 4 (list command name) | DEFER | pre-existing human decision |
| 5 (close vs tab-close naming) | DEFER | adding `session-*` aliases is a behavior change; consistent with Issue 4 deferral |
| 6 (list --json) | DEFER | pre-existing human decision |
| 7 (URL redirect notice) | DEFER | pre-existing human decision |
