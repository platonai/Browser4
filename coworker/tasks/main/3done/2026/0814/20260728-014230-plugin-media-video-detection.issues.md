...
| Issue | Status | Change |
|---|---|---|
| 1 (snake_case→camelCase) | Fixed | MCPToolController.snakeToCamelCase + round-trip test |
| 2 (plugin-<domain> method selection) | Fixed | resolve_plugin_method + tests |
| 3 (bare 'plugin' command) | Deferred | UX |
| 4 (plugin status inactive) | Deferred | plugins are source modules now; status path changed |
| 5 (short flag -v) | Fixed | manual arg parsing |
| 6 (iframe video detection) | Deferred | plugin-specific, needs integration testing |
| 7 (version mismatch) | Fixed | live server version |
| 8 (help lacks plugin-<domain>) | Deferred | discoverability docs |
