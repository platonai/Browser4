...
htmlsnapshot-related issues verified: export now accepts a positional path + --file (commands.rs ArgDef "file"); X-SQL 417 is surfaced by the CLI with workarounds; "Finding browser4 root" removed; CRLF/short-flags/version fixed; --selector on snapshot is a backend ariaSnapshot limitation (no selector param). Remaining are deferred: text truncation (likely page-level CSS ellipsis), character-encoding edge cases, inspect discoverability docs, --count docs, X-SQL 417 pipeline unification (needs integration testing).
