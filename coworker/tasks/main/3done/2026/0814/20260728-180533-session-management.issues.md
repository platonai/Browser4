...
Environment issues verified fixed across these reports: CRLF (LF), short-flag interception (b4w.ps1 manual parsing), b4w.sh nag (removed), version mismatch (live server + SNAPSHOT), "Finding browser4 root" (removed), snapshot -s conflict (short:None), session list/redirect display (verified working). Remaining items are UX/docs (tab-title/tab-active marker, scroll --to-top, screenshot --stdout, per-command help, goto redirect prominence, list --json) or scenario-specific — deferred. Core session/tab/navigation/snapshot functionality verified working.
