...
Attach-related issues verified: short-flag interception (b4w.ps1 manual arg parsing), CRLF (LF), b4w.sh nag (removed), version mismatch (live server + SNAPSHOT), "Short flags detected" warning (removed) — all fixed. The core attach --cdp defect (reuses managed session instead of connecting to external CDP tabs) and named-session ID sharing are deep CDP-connection architecture issues requiring real-Chrome integration testing; deferred. Remaining are UX/docs (state-save --output-dir, tab-new index, snapshot truncation).
