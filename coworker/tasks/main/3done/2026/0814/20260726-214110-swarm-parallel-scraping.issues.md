...
| Issue | Status | Change |
|---|---|---|
| 1 (H2 session race) | Partial | `responseCache.maximumSize` fixed (100→100_000, via bulk-scale-routing); full H2 session-lifecycle fix needs integration testing |
| 2 (failed isDone/error) | Fixed | `ScrapeResponse.failed()` already sets `isDone = true` |
| 3 (submit vs query naming) | Deferred | design change; comparison table is optional low-cost follow-up |
| 4 (--wait docs) | DEFER | pre-existing human decision |
| 5 (debug leak) | Fixed | "Finding browser4 root" message no longer present |
| 6 (pageContentBytes) | Deferred | low-priority UX polish (human-readable size) |
