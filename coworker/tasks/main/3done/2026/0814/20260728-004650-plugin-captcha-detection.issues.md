...
| Issue | Status | Change |
|---|---|---|
| 1 (CAPTCHA @Bean not processed) | Fixed | plugins are now source-tree modules (`browser4-plugins/`) with `getToolExecutors()` override; standard Spring scanning |
| 2 (plugin tools not discoverable) | Partial | `plugin-<domain> <method>` works via resolve_plugin_method; help still doesn't list plugin tools |
| 3 (b4w.ps1 short flags) | Fixed | manual arg parsing |
| 4 (image CAPTCHA false positives) | Deferred | needs plugin integration testing |
| 5 (plugin install workflow) | Deferred | UX |
| 6 (dynamic command single tool) | Fixed | resolve_plugin_method + 5 tests |
| 7 (stale runtime) | Fixed | live server version comparison |
| 8 (--replace flag) | Deferred | UX |
