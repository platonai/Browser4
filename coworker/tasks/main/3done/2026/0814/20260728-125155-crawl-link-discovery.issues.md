...
| Issue | Status | Change |
|---|---|---|
| 1 (TaskLoops bean) | Fixed | AgenticContext.taskLoops has graceful fallback |
| 2 (can't complete task) | Fixed | consequence of Issue 1 |
| 3 (CRLF) | Fixed | b4w.ps1 is LF |
| 4 (protocol handler not ready) | Fixed | CrawlService has MAX_FETCH_RETRIES=3 with backoff |
| 5 (X-SQL 417 vs crawl) | Partial | CLI surfaces 417 with workarounds; pipeline unification needs integration testing |
| 6 (startup delay) | Deferred | needs scheduler investigation |
| 7 (X-SQL naming) | Deferred | docs |
| 8 (--format without --sql) | Fixed | crawl warns when --format has no effect without --sql |
| 9 (stderr noise) | Fixed | "Finding browser4 root" removed |
