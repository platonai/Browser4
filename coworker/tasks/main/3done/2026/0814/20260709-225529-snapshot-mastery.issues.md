...
| Issue | Status | Change |
|---|---|---|
| 1 (`-s` conflicts with global) | Fixed | snapshot `--selector` has `short: None` |
| 2 (`--selector` no effect) | Deferred | backend `ariaSnapshot(viewports, boxes)` has no selector param; needs backend selector-filtering enhancement |
| 3 (`--auto-diff` stdout) | Deferred | UX; diff only rendered in file mode |
| 4 (goto auto-capture interference) | Deferred | design interaction with --auto-diff baseline |
| 5 (session disconnect) | Deferred | needs investigation |
| 6 (SKILL.md --selector limitation) | Deferred | should document once selector filtering is decided |
| 7 (grep --selector no effect) | Deferred | same backend limitation as Issue 2 |
| 8 (snapshot vs htmlsnapshot) | Deferred | docs organization |
