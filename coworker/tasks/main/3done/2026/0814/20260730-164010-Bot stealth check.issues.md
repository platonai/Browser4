...
| Issue | Status | Change |
|---|---|---|
| 1 (Google CAPTCHA) | DEFER | pre-existing human decision |
| 2 (fill --submit) | Deferred | needs real-browser integration testing (Google-specific JS form handling) |
| 3 (fill timeout) | Deferred | needs real-browser integration testing (stuck page-state recovery) |
| 4 (session lost after tab) | Deferred | needs investigation of CLI session tracking across tab ops |
| 5 (tab-list stale URL) | Deferred | needs investigation of tab URL tracking vs actual browser state |
| 6 (htmlsnapshot export syntax) | Fixed | already supports positional path + `--file` (verified in commands.rs) |
| 7 (page context stale) | Deferred | needs investigation of htmlsnapshot vs live-page session reference |
| 8 (browser4-cli vs b4w.ps1) | Resolved | SKILL.md already has the substitution table (lines 15-24) |
| 9 (hardwareConcurrency workers) | Deferred | Web-Worker stealth patch needs deep investigation |
| 10 (screenshot --full-page) | Fixed | `--full-page` flag already implemented (commands.rs) |
| 11 (help overwhelming) | Deferred | low-priority UX; Common workflows section already exists |
