...
| Issue | Status | Change |
|---|---|---|
| 1 (agent result empty {}) | Fixed | `AgentTaskStatus.toCommandStatus()` now transfers `instructResults` into `commandResult` (mirrors `PageVisitStatus`); test added (6 pass) |
| 2 (CRLF line endings) | Already fixed | b4w.ps1/b4w.sh are now LF |
| 3 (b4w.sh JSON quoting) | Already fixed | b4w.sh wraps args in single quotes |
| 4 (--schema custom format) | Already fixed | help states both standard JSON Schema and compact fields format are supported |
| 5 (PowerShell -v/-i) | Already fixed | b4w.ps1 uses manual arg parsing |
| 6 (--schema @file) | Already fixed | main.rs resolves @file for --schema |
| 7 (mixed-language errors) | Already fixed | no Chinese error strings remain in backend .kt |
| 8 (version mismatch) | Already fixed | compares live server version; SNAPSHOT treated as matching |
| 9 (b4w.sh nag) | Already fixed | nag message removed |
| 10 (agent.md sequential IDs) | Fixed | examples updated to `<task-id>` / UUID format |
| 11 (first-run build latency) | Already fixed | SKILL.md documents the Maven build step |
| 12 (agent list table wrap) | Already fixed | DESCRIPTION column is truncated with … |
