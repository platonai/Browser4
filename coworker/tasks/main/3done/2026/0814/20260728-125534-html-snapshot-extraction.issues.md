...
> **Source:** `20260728-125534-html-snapshot-extraction.full.md` | **Date:** 20260728-125534 | **Mode:** dev
...
### Issue 1: b4w.ps1 has CRLF line endings — breaks shebang on Linux
...
On Linux: `./b4w.ps1 help` → `/usr/bin/env: 'pwsh\r': No such file or directory`

#### Expected Behavior

b4w.ps1 should execute correctly on Linux if pwsh is available, or produce a clear error message explaining platform requirements.

#### Actual Behavior

The shebang `#!/usr/bin/env pwsh\r` includes a carriage return character from Windows CRLF line endings, causing the shell to look for a binary called `pwsh\r` which doesn't exist. The error message is confusing for users who have pwsh installed.

#### Root Cause Analysis

The b4w.ps1 file uses Windows-style CRLF line endings. On Linux, the kernel processes the shebang line literally, including the trailing \r as part of the interpreter name. The SKILL.md documents `./b4w.sh` as the Linux wrapper, but the error is cryptic and the `.ps1` file appears to be the primary entry point from the documentation.

#### Code Pointer

`b4w.ps1:1 — shebang line with CRLF line endings`

#### AI Suggested Improvement

- Convert b4w.ps1 to LF line endings (add a .gitattributes entry or run dos2unix)
- Add a pre-commit hook to enforce LF line endings on shell scripts
- Alternatively, add a shebang check at the top of b4w.sh that detects CRLF in b4w.ps1 and warns the user
...
### Issue 2: Text extraction shows CSS text-overflow truncated values instead of full text
...
`./b4w.sh htmlsnapshot get text "article.product_pod h3 a"` → `A Light in the ...` (truncated) instead of `A Light in the Attic`

#### Expected Behavior

Text extraction should return the full text content of the element, matching what `textContent` would return, or at minimum surface a hint that the extracted text is truncated.

#### Actual Behavior

The extracted text matches the visually rendered text (which includes CSS `text-overflow: ellipsis`), not the element's full `textContent`. Users must know to extract the `title` attribute instead.

#### Root Cause Analysis

The htmlsnapshot's text extraction appears to use the rendered/visible text rather than the DOM's `textContent`. This is likely by design for some use cases, but the truncation is not surfaced to the user — there's no hint that more text exists.

#### AI Suggested Improvement

- Surface a truncation hint when extracted text ends with '...' and suggest trying `get attr` to retrieve the full value from attributes like `title` or `alt`
- Consider offering a `--full-text` flag that uses `textContent` instead of visible text
- Add a tip in the output when truncated text is detected
...
### Issue 3: htmlsnapshot export silently ignores unrecognized flags (--output vs --file)
...
`./b4w.sh htmlsnapshot export --output .test-sessions/books-toscrape.html` → saves to default path `.browser4-cli/snapshot/htmlsnapshot-....html` instead of the specified path, with no warning or error.

#### Expected Behavior

Unrecognized flags should produce an error (e.g., 'unknown option: --output') or at minimum a warning on stderr. Alternatively, `--output` should be added as an alias for `--file` since it's the conventional flag name users expect.

#### Actual Behavior

The `--output` flag was silently ignored. The export succeeded but saved to the default location, which could cause data loss if the user doesn't notice.

#### Root Cause Analysis

The CLI's argument parser does not reject unknown flags — it silently drops them. Combined with the fact that `--output` is the conventional GNU/POSIX flag for specifying an output file, users naturally reach for it.

#### Code Pointer

`cli/browser4-cli/src/ — argument parsing logic that should reject unknown flags`

#### AI Suggested Improvement

- Add `--output` as an alias for `--file` in the export command
- Reject unknown flags with a clear error message rather than silently ignoring them
- Add a convention note in the help output explaining that `--file` is used for output paths
...
### Issue 4: htmlsnapshot grep lacks built-in count mode
...
`./b4w.sh htmlsnapshot grep "price"` shows matching lines but provides no count summary. Users must pipe to external tools (grep -c, wc -l) to count occurrences.

#### Expected Behavior

A grep tool should offer a `--count`/`-c` flag that outputs the number of matching lines or occurrences, similar to standard `grep -c`.

#### Actual Behavior

No count flag exists. The output is paginated by default, requiring `--all` to disable pagination, then shell piping to count. The pagination footer says 'showing 20 of 64 matches' which provides a line count but no occurrence count within lines.

#### Root Cause Analysis

The htmlsnapshot grep feature was designed for content search/display, not for counting. The line count in the pagination footer is the closest built-in feature.

#### AI Suggested Improvement

- Add `--count`/`-c` flag to htmlsnapshot grep that outputs just the count
- Add `--count-matches` flag that counts all occurrences (not just matching lines)
- Include match count in the pagination footer even when `--all` is used
...
### Issue 5: No auto-discovery hint when text extraction returns truncated values
...
Run `htmlsnapshot get text` on any element where CSS `text-overflow: ellipsis` is applied. The truncated output looks like the full value to a new user.

#### Expected Behavior

When text extraction returns content ending with '...', the CLI should surface a tip suggesting the user try `get attr` to retrieve the `title` attribute (which often contains the full text), or show a truncation indicator.

#### Actual Behavior

No hint is provided. A new user would assume 'A Light in the ...' is the actual title text.

#### Root Cause Analysis

The htmlsnapshot text extraction doesn't differentiate between actual text ending in '...' (like a deliberate ellipsis in a title) and CSS `text-overflow: ellipsis` truncation. The system doesn't know it's truncated.

#### AI Suggested Improvement

- In the HTML snapshot capture phase, detect elements where `offsetWidth < scrollWidth` (CSS text-overflow) and flag them
- When `get text` returns content ending with '...' for a flagged element, add a stderr tip: 'This element's visible text is CSS-truncated. Try `get attr ... title` for the full text.'
- The inspect output already shows truncated text; add a visual indicator (e.g., '…' vs '...' for deliberate ellipsis)
...
### Issue 6: htmlsnapshot grep default pagination requires --all for complete results
...
`./b4w.sh htmlsnapshot grep "price"` shows only 20 of 64 matches and requires the user to know about `--all` to get the full result set.

#### Expected Behavior

The pagination should either be documented more prominently in the grep help, or `--all` should be the default for grep (since grep is typically used for searching, not browsing).

#### Actual Behavior

Default pagination at 20 matches requires extra flag to disable. The pagination footer mentions `--all` but a new user might miss it.

#### Root Cause Analysis

Grep inherits the same pagination default as `get html` commands, but grep's primary use case (searching) differs from browsing HTML output.

#### AI Suggested Improvement

- Consider making `--all` the default for `htmlsnapshot grep` since the primary use case is searching
- Alternatively, print the pagination footer more prominently when results are truncated
- Add a tip: 'Use --all to see all matches' in the output
...
**Completion Status:** Successful — All 10 task steps completed. The task was achievable with browser4-cli using documented commands, with minor workarounds (using title attr for full text, --file instead of --output for export, shell pipe for counting).

**Success Rate:** 100% — Every step produced the expected result, though some required exploration of flags (export --file) or attribute-based workarounds (title for full text).

**Issues Found:** 6

**Major Blockers:** The b4w.ps1 CRLF issue is a blocker for first-time Linux users who follow the task instructions literally and try `./b4w.ps1`. The ./b4w.sh wrapper works correctly, and the SKILL.md documents it as the Linux option, but the task template says to use `./b4w.ps1`. The discrepancy between the task template and the actual platform support is confusing.

**Most Confusing Aspects:** 1) Which wrapper script to use (b4w.ps1 vs b4w.sh vs b4w.bat) — the task template says b4w.ps1 but the SKILL.md correctly identifies b4w.sh for Linux. 2) Why text extraction shows truncated values — took investigation to discover it's CSS text-overflow and that the title attribute has the full text. 3) The export --output flag being silently ignored — it's not obvious that --file is the correct flag without running --help.

**Most Valuable Improvements:** 1) Fix CRLF line endings on b4w.ps1 for cross-platform compatibility. 2) Surface text truncation hints when extracted text ends with '...'. 3) Reject unknown CLI flags instead of silently ignoring them (or add common aliases). 4) Add --count flag to htmlsnapshot grep for occurrence counting.
...
All issues marked DEFER by human review — no fixes required.
