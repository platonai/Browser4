...
Crawl core issues verified: TaskLoops bean (AgenticContext.taskLoops fallback), protocol-handler retry (CrawlService MAX_FETCH_RETRIES=3 + backoff), --format-without---sql warning, --out-link-pattern quote trim, crawl depth labels (configuredUrl + seed depth 0), CRLF, version mismatch (live server + SNAPSHOT), short flags, and "Finding browser4 root" removal are all fixed. Remaining crawl deep issues (X-SQL 417 pipeline unification, crawl startup-delay scheduler, --parse empty titles, --no-norm/--readonly effect) need integration testing and are deferred.
