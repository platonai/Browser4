...
| Issue | Status | Change |
|---|---|---|
| 1 (attach --cdp doesn't connect) | Deferred | deep CDP connection architecture; needs real-Chrome integration testing |
| 2 (session name collision) | Deferred | UX; no attach --force |
| 3 (named sessions share ID) | Deferred | session-isolation architecture decision |
| 4 (no visual feedback) | Deferred | UX enhancement |
| 5 (short flag warning) | Fixed | "Short flags detected" warning no longer present |
| 6 (version SNAPSHOT mismatch) | Fixed | normalize_version ignores -SNAPSHOT suffix |
| 7 (tab-select --wait) | Deferred | docs gap |
| 8 (help attach category) | Deferred | UX |
