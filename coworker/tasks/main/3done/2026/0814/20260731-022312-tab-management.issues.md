...
Resolved the unresolved git merge conflicts (the `<<<<<<<< HEAD` / `=======` / `>>>>>>>>` markers from an earlier interrupted merge against `3done/2026/0804/tab-management-issues.2.md`) and reconciled the duplicate Human Review checkboxes. All five issues were verified as already implemented in the current codebase:

| Issue | Status | Change |
|---|---|---|
| 1 (tab insert index docs) | Fixed | help.rs (tab-new) documents index-0 insertion and recommends `tab-list` |
| 2 (tab-list active indicator) | Fixed | main.rs renders a `▶` prefix on the active tab row |
| 3 (tab-select page title) | Fixed | tab-select output now prints `Switched to tab N: title — url` |
| 4 (tab-new GUID) | Fixed | tab-new prints `Created tab with GUID: … (url)` |
| 5 (snapshot verbosity) | Fixed | `snapshot --brief` outputs only page URL/title |
