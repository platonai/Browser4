...
**Summary:** - Add a `--stealth` flag to `goto` that launches Chrome with anti-detection flags (disable `--enable-automation`, override `navigator.webdriver`, use a realistic user-agent)
...
**Summary:** - Add a `--react` or `--spa` flag to `fill`/`click` that dispatches React-compatible events (both `input` + `change` events with proper `nativeEvent` simulation)
...
**Summary:** - Add content validation: if fetched HTML is <5KB or matches known CAPTCHA patterns, report it as a fetch error rather than success
...
**Summary:** - Make `snapshot` default to `-v 0` (showing the first viewport with refs), and add a `--full` flag for full captures
...
**Summary:** - Add a "Running from source" section at the top of `--help` output when the binary detects it's running via `cargo run`
...
**Summary:** - Add a `--dedup` flag to `get all` to deduplicate results by value
