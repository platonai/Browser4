...
**Summary:** - On `tab-select`, detect whether `__pulsar_utils__` is available in the page context; if not, re-inject the Browser4 scripts
...
**Summary:** - Add `--no-switch` flag to `tab-new` to preserve current behavior, but default to auto-switching
