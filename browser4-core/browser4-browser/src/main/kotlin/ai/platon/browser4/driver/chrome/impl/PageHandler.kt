...
class PageHandler(
    ...
    val snapshotService: SnapshotService by lazy { CDPSnapshotService(browserProtocol) }

    val jsHandler: JsHandler = JsHandler(browserProtocol, this, isolatedWorldManager)
    ...
    private suspend fun resolveCSSSelectorAll(selector: String): List<NodeRef>? {
        ...
        // Optimized: map directly to NodeRefs without resolving each one via BrowserProtocol if possible.
    ...
    suspend fun scrollIntoViewIfNeeded(locator: String, rect: Rect? = null): NodeRef? {
        ...
        // Prefer smooth behavior when rect is not specified; otherwise honor rect via BrowserProtocol first
        return try {
            if (rect == null) {
                // Try smooth scrolling via JS on the element itself
                if (trySmoothScroll(node)) return node
            }
            // Fallback or rect path: use BrowserProtocol DOM API
    ...
    suspend fun scrollIntoViewIfNeeded(nodeRef: NodeRef, selector: String? = null, rect: Rect? = null): NodeRef? {
        ...
        // If a rect is provided, honor it via BrowserProtocol; otherwise prefer smooth behavior via JS
        return try {
            if (rect != null) {
    ...
    /**
     * Normalize an element locator for use in non-JS contexts (e.g. BrowserProtocol DOM APIs) by extracting a CSS selector if possible.
     * */
     ...
     * @throws ChromeDriverException If an unexpected BrowserProtocol error occurs.
     */
    @Throws(ChromeDriverException::class)
