...
/**
 * [Anti-Captcha](https://anti-captcha.com) CAPTCHA solving provider.
 *
 * Anti-Captcha offers a JSON API very similar to 2Captcha's, supporting
 * reCAPTCHA, hCaptcha, Turnstile, image CAPTCHAs, and FunCaptcha.
 *
 * API documentation: https://anti-captcha.com/apidoc
 */
open class AntiCaptchaProvider(
    ...
    private val logger = getLogger(AntiCaptchaProvider::class)

    override val serviceProvider: CaptchaServiceProvider
        get() = CaptchaServiceProvider.ANTI_CAPTCHA
    ...
    companion object {
        ...
        private const val GET_TASK_URL = "https://api.anti-captcha.com/getTaskResult"
        private const val BALANCE_URL = "https://api.anti-captcha.com/getBalance"
    }
    ...
    override suspend fun solve(request: CaptchaSolveRequest): CaptchaSolution {
        ...
        return try {
            ...
            if (errorId != 0) {
                ...
                return CaptchaSolution.failed(CaptchaServiceProvider.ANTI_CAPTCHA, errorDesc)
            }

            val taskId = createResponse["taskId"]?.toString()
                ?: return CaptchaSolution.failed(CaptchaServiceProvider.ANTI_CAPTCHA, "No taskId in response")

            val token = pollForResult(taskId, startTime)
            val elapsedMs = System.currentTimeMillis() - startTime
            CaptchaSolution.solved(
                provider = CaptchaServiceProvider.ANTI_CAPTCHA,
                token = token,
                taskId = taskId,
                solveTimeMs = elapsedMs
            )
        } catch (e: Exception) {
            logger.warn("Anti-Captcha solve failed: {}", e.message)
    ...
    override suspend fun reportBad(solution: CaptchaSolution) {
        ...
        try {
            ...
            postJson<Map<String, Any?>>("https://api.anti-captcha.com/reportIncorrect", body)
