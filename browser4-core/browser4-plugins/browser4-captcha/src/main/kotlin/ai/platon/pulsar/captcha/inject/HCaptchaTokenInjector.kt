...
/**
 * Injects a solved hCaptcha token into the page via the hcaptcha API.
 */
open class HCaptchaTokenInjector : CaptchaTokenInjector {
    private val logger = getLogger(HCaptchaTokenInjector::class)
    ...
    override suspend fun inject(
        ...
        return try {
            val js = CaptchaSolveScripts.INJECT_HCAPTCHA_TOKEN(token)
            ...
            logger.warn("Failed to inject hCaptcha token: {}", e.message)
            false
        }
    }
}
